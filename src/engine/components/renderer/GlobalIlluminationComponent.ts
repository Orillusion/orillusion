...
export class GlobalIlluminationComponent extends ComponentBase {
    public probes: Probe[];
    public volume: DDGIIrradianceVolume;
    ...
    constructor() {
        super();
        this.componentType = ComponentType.globalIllumination;
    }

    protected start(): void {
        this.volume = GlobalBindGroup.getLightEntries(this.transform.scene3D).irradianceVolume;
        this.initProbe();
    }

    private initProbe() {
        let xCount: number = this.volume.setting.probeXCount;
        let yCount: number = this.volume.setting.probeYCount;
        let zCount: number = this.volume.setting.probeZCount;
        ...
        for (let x = 0; x < xCount; x++) {
            for (let y = 0; y < yCount; y++) {
                for (let z = 0; z < zCount; z++) {
                    ...
                    this.volume.calcPosition(x, y, z, position);

                    probe.x = position.x;
                    probe.y = position.y;
                    probe.z = position.z;

                    probe.transform.rotationX = 0;
                    probe.transform.rotationY = 0;
                    probe.transform.rotationZ = 0;
        ...
        for (let i = 0; i < this.probes.length; i++) {
            EntityCollect.instance.addGIProbe(this.transform.scene3D, this.probes[i]);
        }

        this.object3D.transform.enable = false;

        if (this.volume.setting.debug) {
            this.debug();
        }
    }
    ...
    private _debugProbeRay(probeIndex: number, array: Float32Array) {
        ...
        for (let i = 0; i < rayNumber; i++) {
            ...
            let start = this.probes[probeIndex].transform.worldPosition.clone();
    ...
    private changeProbesPosition(): void {
        ...
        for (let x = 0; x < xCount; x++) {
            for (let y = 0; y < yCount; y++) {
                for (let z = 0; z < zCount; z++) {
                    let index = x + z * xCount + y * (xCount * zCount);
                    let probe: Probe = this.probes[index];
                    this.volume.calcPosition(x, y, z, position);
