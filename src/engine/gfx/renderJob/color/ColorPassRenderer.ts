...
export class ColorPassRenderer extends RendererBase {
    ...
    public render(view: View3D, occlusionSystem: OcclusionSystem, maskTr: boolean = false) {
        // return ;
...

        {
            ...
            if (!maskTr && EntityCollect.instance.sky) {
                GPUContext.bindCamera(renderPassEncoder, camera);
                EntityCollect.instance.sky.renderPass2(view, this._rendererType, this.rendererPassState, this.clusterLightingRender, renderPassEncoder);
            }

            GPUContext.bindCamera(renderPassEncoder, camera);
            this.drawNodes(view, this.renderContext, collectInfo.opaqueList, occlusionSystem);
...

        {
            ...
            if (!maskTr) {
                GPUContext.bindCamera(renderPassEncoder, camera);
                this.drawNodes(view, this.renderContext, collectInfo.transparentList, occlusionSystem);
            }
            ...
            for (let i = 0; i < graphicsList.length; i++) {
                ...
                graphic3DRenderNode.nodeUpdate(view, this._rendererType, this.splitRendererPassState, this.clusterLightingRender);
                graphic3DRenderNode.renderPass2(view, this._rendererType, this.splitRendererPassState, this.clusterLightingRender, renderPassEncoder);
            }
    ...
    public drawNodes(view: View3D, renderContext: RenderContext, nodes: RenderNode[], occlusionSystem: OcclusionSystem) {
        {
            for (let i = Engine3D.setting.render.drawOpMin; i < Math.min(nodes.length, Engine3D.setting.render.drawOpMax); ++i) {
                ...
                renderNode.nodeUpdate(view, this._rendererType, this.rendererPassState, this.clusterLightingRender);
