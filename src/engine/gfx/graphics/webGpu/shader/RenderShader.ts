...
export class RenderShader extends ShaderBase {
    ...
    private static _renderShaderModulePool: Map<string, GPUShaderModule> = new Map<string, GPUShaderModule>();
    private static _renderShader: Map<string, RenderShader> = new Map<string, RenderShader>();
    private _vs_limit = [];
    private _fs_limit = [];
    private _cs_limit = [];
    ...
    public static createShader(vs: string, fs: string): string {
        ...
        RenderShader._renderShader.set(shader.instanceID, shader);
    ...
    public static destroyShader(instanceID: string) {
        if (RenderShader._renderShader.has(instanceID)) {
            let shader = RenderShader._renderShader.get(instanceID);
            shader.destroy();
            RenderShader._renderShader.delete(instanceID)
        }
    }
    ...
    public static getShader(instanceID: string) {
        return RenderShader._renderShader.get(instanceID);
    }
    ...
    protected compileShader(stage: ShaderStage, code: string, renderPassState: RendererPassState) {
        ...
        let shaderModule = RenderShader._renderShaderModulePool.get(key);
        if (!shaderModule) {
            shader = this.applyPostDefine(shader, renderPassState);
            ...
            RenderShader._renderShaderModulePool.set(key, shaderModule);
        }
            ...
            if (info.varType == `uniform`) {
                ...
                switch (info.dataType) {
                    case `sampler`:
                        {
                            ...
                            let texture = this.textures[textureName] ? this.textures[textureName] : defaultRes.redTexture;
                            let entry: GPUBindGroupLayoutEntry = {
                                binding: info.binding,
                    ...
                    case `texture_depth_cube_array`:
                        {
                            let texture = this.textures[info.varName] ? this.textures[info.varName] : defaultRes.redTexture;
                            let entry: GPUBindGroupLayoutEntry = {
                                binding: info.binding,
                                visibility: texture.visibility,
        ...
        if (!this.bindGroups[groupIndex] || force) {
            ...
            for (let j = 0; j < shaderRefs.length; j++) {
                ...
                if (refs.varType == `uniform`) {
                    ...
                    if (buffer) {
                        if (buffer instanceof MaterialDataUniformGPUBuffer) {
                    ...
                    if (refs.dataType == `sampler`) {
                        ...
                        if (!texture) {
                            texture = defaultRes.blackTexture;
                        ...
                        if (!texture) {
                            texture = defaultRes.whiteTexture;
