...
export class Object3D extends Entity {
     ...
     */
    public addComponent<T extends ComponentBase>(c: Ctor<T>, param?: any): T {
        if (!this.components.has(c.prototype)) {
            let instance: T = new c() as T;
            instance.object3D = this;
            instance[`__init`](param);
            // instance[`__start`]();
            this.components.set(c.prototype, instance);
    ...
    private appendLateStart(component: ComponentBase) {
        let arr = ComponentBase.waitStartComponent.get(this);
        if (!arr) {
            ComponentBase.waitStartComponent.set(this, [component]);
        } else {
     ...
     */
    public getOrAddComponent<T extends ComponentBase>(c: Ctor<T>): T {
        let component = this.components.get(c.prototype);
        if (!component) {
     ...
     * @param c class of component
     */
    public removeComponent<T extends ComponentBase>(c: Ctor<T>) {
        if (this.components.has(c.prototype)) {
            let component = this.components.get(c.prototype);
            this.components.delete(c.prototype);
            component[`__stop`]();
            component.destroy();
        }
    }
     ...
     * @returns boolean
     */
    public hasComponent<T extends ComponentBase>(c: Ctor<T>): boolean {
        return this.components.has(c.prototype);
    }
     ...
     * @returns result component
     */
    public getComponent<T extends ComponentBase>(c: Ctor<T>): T {
        return this.components.get(c.prototype) as T;
    }
     ...
     */
    public getComponentFromParent<T extends ComponentBase>(c: Ctor<T>): T {
        if (!this.parent) {
            return null;
        }
     ...
     */
    public getComponentsInChild<T extends ComponentBase>(c: Ctor<T>): T[] {
        let list: T[] = [];
        let component = this.components.get(c.prototype);
        if (component) {
     ...
     * @returns {outList}
     */
    public getComponents<T extends ComponentBase>(c: Ctor<T>, outList?: Array<T>, includeInactive?: boolean): T[] {
        outList ||= [];
     ...
     */
    public getComponentsExt<T extends ComponentBase>(c: Ctor<T>, ret?: T[], includeInactive?: boolean): T[] {
        if (!ret) ret = [];
        let component = this.components.get(c.prototype);
        if (component && (component.enable || includeInactive)) {
