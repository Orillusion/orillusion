...
export class Entity extends CEventDispatcher {
     ...
     */
    public components: Map<any, ComponentBase>;
    /**
     *
    ...
    protected waitDisposeComponents: ComponentBase[];
    ...
    constructor() {
        super();
        this.entityChildren = [];
        this.components = new Map<any, ComponentBase>();
    ...
    public waitUpdate(): void {
        if (this._dispose) {
            ...
            ComponentBase.waitStartComponent.forEach((v, k) => {
                v.forEach((v) => {
                    v[`__start`]();
                })
                ComponentBase.waitStartComponent.delete(k);
            });
        }
    }
