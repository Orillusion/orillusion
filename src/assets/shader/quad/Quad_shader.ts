export class Quad_shader {

    public static FullQuad_vert_wgsl: string = /* wgsl */ `
      ...
      fn main(@builtin(vertex_index) vertexIndex : u32, @builtin(instance_index) index : u32 ) -> VertexOutput {
        ...
        //   var pos = (worldMatrix[3].xy / windowSize) * vec2<f32>(1.0,-1.0) ;

        //   uvPos = uvPos * size;

        //   let screenPos = vec2(((uvPos * 2.0) - vec2<f32>(1.0))) ;
    ...
    public static Quad_vert_wgsl: string = /* wgsl */ `
      #include "WorldMatrixUniform"
      #include "GlobalUniform"
      ...
      fn main(@builtin(instance_index) index : u32,@location(0) position: vec3<f32>, @location(1) TEXCOORD_1: vec2<f32>) -> VertexOutput {
        ...
        //   let offset = (size) / windowSize * 0.5 + pos / windowSize ;

          let uv = vec2(((TEXCOORD_1.xy * 2.0) - vec2<f32>(1.0)))  ;// / windowSize * size - offset ;
        //   let suv = worldMatrix * vec4<f32>(uv, 0.0, 1.0)  ;
    ...
    public static Quad_frag_wgsl: string = /* wgsl */ `
    ...
    public static Quad_depth2d_frag_wgsl: string = /* wgsl */ `
        struct FragmentOutput {
            @location(0) o_Target: vec4<f32>
        }; 
    ...
    public static Quad_depthCube_frag_wgsl: string = /* wgsl */ `
        struct FragmentOutput {
            @location(0) o_Target: vec4<f32>
        };
    ...
    public static Quad_depth2dArray_frag_wgsl: string = /* wgsl */ `
    struct FragmentOutput {
        @location(0) o_Target: vec4<f32>
    };
