export class constraint {
    public static cs: string =  /* wgsl */ `
        ...
        @group(0) @binding(2) var<storage, read_write> tmpposition: array<vec4<f32>>;
        @group(0) @binding(3) var<storage, read_write> velocity: array<vec4<f32>>;
        @group(1) @binding(0) var<storage, read_write> anchorposition: array<vec4<f32>>;
        @group(1) @binding(1) var<storage, read_write> newposition: array<vec4<f32>>;
        ...
        fn CsMain(
            ...
            var oldposition = vec3<f32>(tmpposition[index][0], tmpposition[index][1], tmpposition[index][2]);
            var tempvelocity = vec3<f32>(velocity[index][0], velocity[index][1], velocity[index][2]);
            if (position[index][3] > 0.0) {
                tempvelocity = tempvelocity + vec3<f32>(0.0, gravity * deltatime, 0.0); 
            }
            ...
            // var temp = vec3<f32>(tmpposition[index][0], tmpposition[index][1], tmpposition[index][2]);
            // position[index][0] = tempposition.x;
            // position[index][1] = tempposition.y;
            // position[index][2] = tempposition.z;
            if(distance(newheadcentre, tempposition) < (headradius - 0.02)){
                var tsphere = intersectSphere(oldposition, tempvelocity, newheadcentre, headradius);
                // output[index][0] = distance(headcentre, tempposition);
                tempposition = oldposition + tempvelocity * tsphere;
            ...
            tempposition = satisfyConstraint(tempposition, anchor, length);
